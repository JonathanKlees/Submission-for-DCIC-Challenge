...
class PLS(AlgorithmSkelton):
    ...
    def preprocess(self, dataframe, dataset_info, paths, v_fold, num_annos, percentage_labeled, mode):   #self, dataset_info, paths, gt, v_fold, num_annos, percentage_labeled
        ...
        # Here, only partial label information may be available, which is not important for zero shot CLIP but if we pass label information to UMAP.
        # However, due to time constaints, we assume the scenario of one annotation per image here to simplify the code.

        # CLIP
        device = "cuda" if torch.cuda.is_available() else "cpu"
        # model, preprocess = clip.load('ViT-B/32', device)
        model, preprocess = clip.load('ViT-L/14', device) # use more powerful CLIP model here to crunch numbers
        ...
        # Here, we supply label information to UMAP to further improve the embedding. Also, we further reduce the target dimension and choose more neighbors
        # UMAP
        def UMAP_reduction(dataset, layer, dim = 50, store = False, dir_name = "./"):
            features = np.array(dataset[layer].to_list())
            features = features.astype(np.float64)
            reducer = umap.UMAP(n_neighbors = 15, min_dist = 0.05, n_components=dim, metric = "euclidean", target_metric = "categorical", target_weight = 0.6, random_state = 0)
            # reducer.fit(features)
            # reduced_features = reducer.transform(features)
            annotations = np.array(dataset["label"].to_list())
            reduced_features = reducer.fit_transform(features, y = annotations) # Here, we could go with fit on labeled data and then transform on all in the case of partial supervision.

            return reduced_features

        #  CLIP + UMAP
        reduced_features_umap = UMAP_reduction(dataset, "CLIP", dim = 5, store = False)
        dataset["CLIP_UMAP_"+str(20)] = list(reduced_features_umap) #  lets still call this UMAP_20 to not have to change too much
